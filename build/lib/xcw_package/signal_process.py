...
def pdf(
    data: np.ndarray, samples: int, plot: bool = False, **Kwargs
) -> np.ndarray:  # 绘制PDF图
...
def stft(
    data: np.ndarray, fs: float, nperseg: int, plot: bool = False, **Kwargs
) -> np.ndarray:
    N = len(data)
    if nperseg > N:
        raise ValueError("段长大于信号长度，无法绘制STFT图")
    num_segment = N // nperseg  # 根据段长确定分段数，多余数据舍去

    fft_matrix = np.zeros((num_segment, nperseg))  # 频谱存储矩阵
    for i in range(num_segment):  # 分段
        st_data = data[i * nperseg : (i + 1) * nperseg]  # 取出段数据
        fft_data = np.abs(fft(st_data)) / nperseg  # 计算短时傅里叶变换幅值
        fft_matrix[i, :] = fft_data  # 将结构存入对应行数
    t = np.arange(0, num_segment) / fs * nperseg
    f = np.linspace(0, fs / 2, nperseg, endpoint=False)
    fft_matrix = fft_matrix[:, : len(f)]  # 只显示正频率
...
#         data = data[i * nperseg : (i + 1) * nperseg]  # 取出段数据
#         fft_data = fft(data)
...
#     # 绘制移动窗图
#     plt.figure(figsize=(12,5))
#     plt.plot(s.t_values[: len(data) - window_length + 1], results)
#     plt.xlabel("t/s")
#     plt.title(title)
#     if result:
#         return results
...
#         bpdata=bandpass(data,f,bw,s.f_s)
